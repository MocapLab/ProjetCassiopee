...
def plot_animation_no_points(i, data, label, prediction, nom, model_type='FC'):
...
if __name__ == "__main__":
    ...
    for i, batch in enumerate(data_loader) :
        ...
        data = data.squeeze(0)
        nom = f'{name}_{label}_{predicted}'

        if name == 'Venir.csv' :
            print(f"Plotting {nom} ...")
            plot_animation_no_points(i, data, label, predicted, nom, model_type='FC')
